...
class KMeansTextClustering:
    ...
    def cluster_documents(self):
        ...
        self.cluster_labels = kmeans.fit_predict(self.vector_space_model.normalized_tfidf_matrix)
    ...
    def evaluate_clustering(self):
        ...
        purity = sum(majority_label == true_label for majority_label, true_label in zip(majority_labels, true_labels)) / len(true_labels)

        silhouette = silhouette_score(self.vector_space_model.normalized_tfidf_matrix, self.cluster_labels)
        rand_index = adjusted_rand_score(true_labels, self.cluster_labels)

        return {'purity': purity, 'silhouette_score': silhouette, 'rand_index': rand_index}
    ...
    def reduce_dimentions(self):
        ...
        reduced_matrix = pca.fit_transform(self.vector_space_model.normalized_tfidf_matrix)
    ...
    def plot_clusters(self):
        ...
        scatter = plt.scatter(reduced_matrix[:, 0], reduced_matrix[:, 1], c=self.cluster_labels, cmap='viridis')
        plt.xlabel('PCA 1')
        plt.ylabel('PCA 2')
...
if __name__ == "__main__":
    kmeans_clustering = KMeansTextClustering(data_dir='./data', index_file='./docs/inv-index.json', k=5)
